...
#     '''
#
#     def __init__(self, n_channel, dim_latent, dim_input, output_dim):
#         super().__init__()
#         # Constant input
#         # Style generators
...
class StyleConv_Block(nn.Module):
    ...
    def __init__(self, in_channel, out_channel, dim_latent, output_dim):
        super().__init__()
        # Style generators
...
class StyleBased_Generator(nn.Module):
    ...
    def __init__(self, n_fc, dim_latent):
        ...
        self.constant = nn.Parameter(torch.randn(1, 512, self.output_dims[0][0], self.output_dims[0][1]))


        self.fcs = Intermediate_Generator(n_fc, dim_latent)
        self.convs = nn.ModuleList([
            StyleConv_Block(512, 512, dim_latent, self.output_dims[0]),
            StyleConv_Block(512, 512, dim_latent, self.output_dims[1]),
            StyleConv_Block(512, 512, dim_latent, self.output_dims[2]),
            StyleConv_Block(512, 512, dim_latent, self.output_dims[3]),
            StyleConv_Block(512, 256, dim_latent, self.output_dims[4]),
            StyleConv_Block(256, 128, dim_latent, self.output_dims[5]),
            StyleConv_Block(128, 64, dim_latent, self.output_dims[6]),
            StyleConv_Block(64, 32, dim_latent, self.output_dims[7]),
            # StyleConv_Block(32, 16, dim_latent, self.output_dims[8]),
            # StyleConv_Block(16, 16, dim_latent, self.output_dims[9])
    ...
    def forward(self, latent_z,
                step=7,  # Step means how many layers (count from 4 x 4) are used to train
        ...
        for i in range(step + 1):
            ...
            noise.append(torch.randn((batch_size, 1, self.output_dims[i][0], self.output_dims[i][1]), device=torch.device('cuda:0')))
        ...
        for i, conv in enumerate(self.convs):
            ...
            if i > 0 and step > 0:
                ...
                #                                             align_corners=False)
                result_upsample = nn.functional.interpolate(result, mode='bilinear',
                                                            align_corners=False,
                                                            size=self.output_dims[i])
                result = conv(result_upsample, current_latent, noise[i])


            # Final layer, output rgb image
            if i == step:
                result = nn.functional.sigmoid(self.to_rgbs[i](result))
...
class Discriminator(nn.Module):
    def __init__(self, feature_size, loss_fct="standard"):
        ...
        self.layer1 = torch.nn.Sequential(
            spectral_norm(torch.nn.Conv2d(1, self.feature_size, kernel_size=3, stride=2, padding=(1, 1))),
            torch.nn.MaxPool2d(kernel_size=(2, 2)),
            torch.nn.ReLU()
        )  # (1, 400, 128)

        self.layer2 = torch.nn.Sequential(
            spectral_norm(torch.nn.Conv2d(self.feature_size, self.feature_size * 2, kernel_size=3, stride=2, padding=(1, 1))),
            torch.nn.MaxPool2d(kernel_size=(2, 2)),
            torch.nn.ReLU()
        )  # (1, 100, 64)

        self.layer4 = torch.nn.Sequential(
            spectral_norm(torch.nn.Conv2d(self.feature_size * 2, self.feature_size * 4, kernel_size=3, stride=(2, 1), padding=(1, 1))),
            torch.nn.MaxPool2d(kernel_size=(2, 2)),
            torch.nn.ReLU()
        )  # (1, 25, 16)

        self.layer5 = torch.nn.Sequential(
            spectral_norm(torch.nn.Conv2d(self.feature_size * 4, self.feature_size * 8, kernel_size=3, stride=2, padding=(0, 1))),
            torch.nn.ReLU()
        )  # (1, 12, 8)
